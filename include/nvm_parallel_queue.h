...
uint32_t cq_poll(nvm_queue_t* cq, uint16_t search_cid) {
    ...
    while (true) {
        ...
        for (size_t i = 0; i < cq->qs_minus_1; i++) {
            ...
            /* if (j % 100 == 0) */
            /*     printf("qs_log2: %llu\thead_shift: %llu\thead_shift_and: %llu\thead_shift_and_flipped: %llu\thead: %llu\tcur_head: %llu\tsearch_cid: %llu\tsearch_phase: %llu\tcq->loc: %p\tcq->qs: %llu\ti: %llu\tj: %llu\tcid: %llu\tphase:%llu\n", */
            /*            (unsigned long long) cq->qs_log2, (unsigned long long) (head >> cq->qs_log2), (unsigned long long) ((head >> cq->qs_log2) & 0x01), (unsigned long long) ~((head >> cq->qs_log2) & 0x01), */
