...
do
    ...
    do
        ...
        do
            ...
            do
                echo "++++++++++++++++++ $C Controllers ++++++++++++++++++"
                ./bin/nvm-cc-bench -f ${GraphFileArray[gfid]} -l ${GraphFileOffset[gfid]} --impl_type $IMPLTYPE --memalloc $MEMTYPE --n_ctrls $C -p $P --gpu $GPU --threads $TB --sector_size 4096 --COARSE 8 --queue_depth 64 --num_queues 128 | grep -e time -e IO -e Hit
            done
        done
    done
done
