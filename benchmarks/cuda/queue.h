...
#define MAX_CQ_ENTRIES_64K  (64*1024/16)
    ...
    QueuePair( const nvm_ctrl_t* ctrl, const uint32_t cudaDevice, const struct nvm_ns_info ns, const struct nvm_ctrl_info info, nvm_aq_ref& aq_ref, const uint16_t qp_id)
    {
        ...
        uint64_t sq_size = 512;
        uint64_t cq_size = 512;
        ...
        std::cout << "Finished creating sq dma "<< this->sq_mem.get()->vaddr << std::endl;
        ...
        std::cout << "Finished creating cq dma  "<< this->cq_mem.get()->vaddr << std::endl;
